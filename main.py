...
def upload_file():
    ...
    if request.method == 'POST':
        ...
        df = pd.read_excel(
            f.filename, sheet_name='รายชื่อQR Coed', usecols="A,B,C")
        os.remove("qrcode/test.zip")
        ...
        for i in range(len(df)):
            ...
            if username not in GenerateUserNamelist:
                GenerateUserNamelist.append(username)
                # connection = getConnection()
                # sql = "INSERT INTO user(username, name, agency) VALUES('%s', '%s', '%s', '%s')" % (username,
                #      name, agency)
                # cursor = connection.cursor()
                # cursor.execute(sql)
                # connection.commit()
        ...
        shutil.make_archive('qrcode/test', 'zip', 'qrcode/test')
